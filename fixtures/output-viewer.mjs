...
export function outputViewerScaleFixture() {
  ...
  return { projectName: "100 devices / 300 wires", cableHops: true, deviceLibrary: [template],
    devices: Array.from({ length: 100 }, (_, i) => ({ instanceId: `scale-${i}`, templateId: template.id,
      name: `Device ${i + 1}`, x: i % 10 * 500, y: Math.floor(i / 10) * 500 })),
    connections: Array.from({ length: 300 }, (_, i) => ({ id: `scale-wire-${i}`, cableType: "sdi",
      from: { deviceId: `scale-${Math.floor(i / 3)}`, connectorId: `output-${i % 3}` },
      to: { deviceId: `scale-${(Math.floor(i / 3) + 1 + i % 3 * 10) % 100}`, connectorId: `input-${i % 3}` } })) };
}
