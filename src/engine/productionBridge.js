...
// Keep Engine startup tolerant of a stale browser-cached hitTest.js module while
// Iteration 48.1 is in active testing. Named imports fail at module-load time if
// a browser has the previous helper cached, so rack hit testing falls back here.
const hitTestRack = typeof HitTest.hitTestRack === "function"
  ? HitTest.hitTestRack
  : fallbackHitTestRack;
...
export const ENGINE_PRODUCTION_BRIDGE_FINGERPRINT = "production-bridge-iteration54-10-1-generated-connector-selection-hardening";
export const ENGINE_BRIDGE_VERSION = "iteration54-10-1-generated-connector-selection-hardening";
export const ENGINE_BRIDGE_FEATURE_LABEL = "generated-connector-selection-hardening";
...
const JUMP_PRESS_HIT_TOLERANCE_PX = 6;
...
class ProductionEngineBridge {
  ...
  start() {
    ...
    if (this.debugObjectSnapping) {
      ensureSnapTraceBuffer({
        bridgeVersion: BRIDGE_VERSION,
        feature: BRIDGE_FEATURE_LABEL,
        snapMode: this.debugSnapMode
      });
    }
    this.bindEvents();
  ...
  snapTrace() {
    return typeof window !== "undefined"
      ? window.__AVD_SNAP_TRACE__ || null
      : null;
  }
  ...
  selectRack(rackId) {
    if (!rackId || !this.scene.getRack(rackId)) return false;
    this.scene.selectRackOnly(rackId);
    this.updateSelectionHud();
    this.updateRackBuilderDebugHud("selectRack API");
    this.scheduleRender();
    return this.scene.isRackSelected(rackId);
  }
  ...
  sceneCounts() {
    ...
    return {
      ...
      selectedRacks: this.scene.selectedRackIds.size,
  ...
  updateJumpNodeDebugSnapshot(reason = "jump debug") {
    ...
    const geometryJumpId = primarySelectedJumpId
      || hoveredJumpId
      || overlays[0]?.outputJumpId
    ...
    const snapshot = {
      ...
      visiblePortal: visiblePortal ? {
        ...
        geometry: visiblePortal.geometry || "",
        sampledPointCount: visiblePortal.sampledPointCount || visiblePortal.points?.length || 0
      } : null,
      jumpPositions: this.scene.devices
        .filter(device => isJumpNodeDevice(device))
        .slice(0, 12)
        .map(device => ({
          id: String(device.id || ""),
          center: this.jumpNodeCenter(device)
        })),
  ...
  recordRackCanvasDiagnostic(step, details = {}) {
    if (!this.debugRackBuilder) return;
    const rackId = String(details.rackId || "");
    const rack = rackId ? this.scene.getRack(rackId) : null;
    const payload = {
      step,
      ...details,
    ...
    console.info(`[engine-rack-canvas] ${step}`, payload);
    this.hud?.setMetric("rack canvas", step);
  }
  ...
  jumpNodeGeometrySnapshot(jumpId = "") {
    const id = String(jumpId || "");
    const device = id ? this.scene.getDevice(id) : null;
    if (!isJumpNodeDevice(device)) return null;
    const center = this.jumpNodeCenter(device);
    const connector = this.scene.getConnector(id, JUMP_NODE_CONNECTOR_ID);
    const connectorPoint = connector ? this.scene.connectorWorldPoint(device, connector) : null;
    const roleInfo = this.scene.jumpNodeRole(id);
    const wire = roleInfo?.localWire || null;
    const wireSide = wire && this.scene.wireEndpointObjectId(wire, "from") === id
      ? "from"
      : wire && this.scene.wireEndpointObjectId(wire, "to") === id
        ? "to"
        : "";
    const wireEndpoint = wireSide ? this.scene.endpointForWire(wire, wireSide) : null;
    const connectorDelta = center && connectorPoint ? pointDistance(center, connectorPoint) : null;
    const wireDelta = center && wireEndpoint ? pointDistance(center, wireEndpoint) : null;
    return {
      id,
      rawCenter: {
        x: Number(device.x || 0) + JUMP_NODE_SIZE / 2,
        y: Number(device.y || 0) + JUMP_NODE_SIZE / 2
      },
      normalizedBounds: {
        x: Number(device.x || 0),
        y: Number(device.y || 0),
  ...
  jumpConnectorHitForDevice(device, { distance = 0, candidates = 0, ms = 0 } = {}) {
    if (!device || !isJumpNodeDevice(device)) return null;
    const connector = (device.connectors || []).find(item => item.id === JUMP_NODE_CONNECTOR_ID)
      || this.scene.getConnector?.(device.id, JUMP_NODE_CONNECTOR_ID)
      || {
        ...
        direction: "io",
        side: "center",
        x: Number(device.width || JUMP_NODE_SIZE) / 2,
        y: Number(device.height || JUMP_NODE_SIZE) / 2
      };
    const point = this.jumpNodeCenter(device);
  ...
  hitTestJumpPressTarget(worldPoint) {
    const start = performance.now();
    const zoom = Math.max(this.camera.zoom || 1, 0.0001);
    const toleranceWorld = JUMP_PRESS_HIT_TOLERANCE_PX / zoom;
    const candidates = this.scene.spatialIndex.queryRect({
      x: Number(worldPoint?.x || 0) - toleranceWorld,
      y: Number(worldPoint?.y || 0) - toleranceWorld,
    ...
    for (let index = candidates.length - 1; index >= 0; index -= 1) {
      const device = candidates[index]?.payload?.device;
      if (!device || !isJumpNodeDevice(device)) continue;
      const center = this.jumpNodeCenter(device);
      if (!center) continue;
      const dxPx = (Number(worldPoint?.x) - center.x) * zoom;
      const dyPx = (Number(worldPoint?.y) - center.y) * zoom;
      const distancePx = Math.hypot(dxPx, dyPx);
      const radiusPx = Math.max(Number(device.width || JUMP_NODE_SIZE), Number(device.height || JUMP_NODE_SIZE)) * zoom / 2;
      if (distancePx <= radiusPx + JUMP_PRESS_HIT_TOLERANCE_PX && distancePx < bestDistancePx) {
        best = device;
    ...
    const connector = this.jumpConnectorHitForDevice(best, {
      distance: bestDistancePx / zoom,
      candidates: candidates.length,
      ms: performance.now() - start
    });
    return { connector, device: best, candidates: candidates.length, ms: performance.now() - start, distancePx: bestDistancePx };
  }

  jumpPressDistancePx(point = null) {
    const pending = this.pendingJumpPress;
    if (!pending) return 0;
    const current = point || pending.lastScreen || pending.startScreen;
    const dx = Number(current?.x || 0) - Number(pending.startScreen?.x || 0);
    const dy = Number(current?.y || 0) - Number(pending.startScreen?.y || 0);
  ...
  beginPendingJumpPress(hit, point, worldPoint, event, additiveSelection = false, options = {}) {
    ...
    this.cancelPendingJumpPress("replace", { updateHud: false });
  ...
  cancelPendingJumpPress(reason = "cancelled", { updateHud = true, winner = "cancelled" } = {}) {
    ...
    if (updateHud) {
      ...
      this.updateJumpNodeDebugSnapshot(`jump-press-${reason}`);
      this.updateCanvasCursor();
    }
    return pending;
  }
  ...
  resolvePendingJumpPressAsMove(event) {
    ...
    if (!pending) return false;
    const jumpDevice = this.scene.getDevice(pending.jumpId);
    const startScreen = pending.startScreen;
    ...
    this.hud?.setMetric("jump press", `${pending.jumpId} move`);
    this.updateJumpNodeDebugSnapshot("jump-press-move-start");
    return true;
  }
  ...
  beginJumpLinkCreate(connectorHit, worldPoint) {
    ...
    const fromPoint = connectorHit?.point || this.jumpNodeCenter(connectorHit?.device);
    if (!jumpId || !fromPoint) return;
    this.jumpLinkCreate = {
      fromJumpId: jumpId,
      fromPoint: { ...fromPoint },
  ...
  updateJumpLinkCreate(worldPoint) {
    ...
    const hit = this.hitTestJumpPressTarget(worldPoint);
  ...
  handlePointerDown(event) {
    ...
    const jumpPressHit = this.hitTestJumpPressTarget(world);
    if (jumpPressHit.connector) {
      const jumpDevice = jumpPressHit.connector.device;
    ...
    const jumpLinkHit = this.hitTestVisibleJumpLink(world, 8);
    if (jumpLinkHit.link) {
      this.clearJumpMoveArm("jump-link-select", { updateHud: false });
      this.clearHoverState("jump-link-select", { render: false });
      this.scene.selectJumpLinkOnly(jumpLinkHit.link.id);
      this.updateSelectionHud();
      this.updateInteractionHud("jump-link-select", jumpLinkHit);
      this.scheduleRender();
      return;
    }
    ...
    if (connectorHit.connector) {
      if (isJumpConnectorHit(connectorHit.connector)) {
        ...
        const fallbackJumpHit = {
          ...connectorHit,
          connector: {
            ...connectorHit.connector,
            point: this.jumpNodeCenter(jumpDevice) || connectorHit.connector.point
          },
          device: jumpDevice
        };
        if (event.shiftKey && connectedEndpoint) {
    ...
    if (!deviceHit.device) {
      ...
      if (rackHit.rack) {
        ...
        if (additiveSelection) {
          this.scene.toggleRackSelection(rack.id);
          this.recordRackCanvasDiagnostic("toggle rack from frame", {
            rackId: rack.id,
            chosenAction: "select rack",
            hitCandidates: rackHit.candidates
          });
          this.updateSelectionHud();
          this.updateInteractionHud("rack-selection-toggle", rackHit);
          this.scheduleRender();
          return;
        }
        if (!wasSelected) this.scene.selectRackOnly(rack.id);
        this.recordRackCanvasDiagnostic("select rack from frame", {
          rackId: rack.id,
          chosenAction: "start rack drag",
          hitCandidates: rackHit.candidates
        });
        this.updateSelectionHud();
    ...
    const rack = rackId ? this.scene.getRack(rackId) : null;
    const actionableRackId = rack?.id || "";
    ...
    if (additiveSelection) {
      ...
      if (rackId) this.recordRackCanvasDiagnostic("toggle child device rack selection", {
        engineChildId: deviceHit.device.id,
        productionChildId: deviceHit.device.sourceId || deviceHit.device.id,
        rackId,
        chosenAction: actionableRackId ? "select rack" : "select child fallback"
      });
      this.updateSelectionHud();
      this.updateInteractionHud(actionableRackId ? "rack-selection-toggle" : "selection-toggle", deviceHit);
      this.scheduleRender();
      return;
    } else if (!wasSelected) {
      if (actionableRackId) this.scene.selectRackOnly(actionableRackId);
      else this.scene.selectOnly(deviceHit.device.id);
    }
    if (rackId) this.recordRackCanvasDiagnostic("child device pointerdown", {
      engineChildId: deviceHit.device.id,
      productionChildId: deviceHit.device.sourceId || deviceHit.device.id,
      rackId,
      chosenAction: actionableRackId ? "start rack drag" : "start child drag fallback"
    });
    this.updateSelectionHud();
    if (actionableRackId) this.updateRackBuilderDebugHud("rack selected");
    this.beginPendingDrag(point, world, event);
  }
  ...
  handlePointerMove(event) {
    ...
    if (this.pendingJumpPress) {
      const world = screenToWorld(this.camera, point);
      const pressIntent = this.updatePendingJumpPress(point, world, event);
      if (pressIntent !== "move") {
        this.hud.setMetric("pointermove", `${(performance.now() - pointerStart).toFixed(3)} ms`);
        this.scheduleRender();
        return;
    ...
    if (this.dragSession) {
      const start = performance.now();
      const world = screenToWorld(this.camera, point);
      this.dragSession.update(world, {
        camera: this.camera,
        snappingEnabled: this.objectSnappingEnabled(),
        axisLockRequested: Boolean(event.shiftKey),
        snapMode: this.debugSnapMode
      });
      this.captureSnapTraceFrame(event, point, world);
  ...
  beginPendingDrag(point, worldPoint, event = null) {
    ...
    this.pendingDrag = {
      ...
      startWorld: { ...worldPoint },
      startClient: {
        clientX: Number.isFinite(Number(event?.clientX)) ? Number(event.clientX) : point.x,
        clientY: Number.isFinite(Number(event?.clientY)) ? Number(event.clientY) : point.y
      },
      selectedIds
    };
  ...
  snapDebugVisualState() {
    ...
    return {
      ...
      rawRect: offsetDebugRect(startRect, this.dragSession.preSnapDx, this.dragSession.preSnapDy),
      finalRect: offsetDebugRect(startRect, this.dragSession.dx, this.dragSession.dy)
    };
  }

  captureSnapTraceFrame(event, point, worldPoint) {
    if (!this.debugObjectSnapping || !this.dragSession) return null;
    const trace = ensureSnapTraceBuffer({
      bridgeVersion: BRIDGE_VERSION,
      feature: BRIDGE_FEATURE_LABEL,
      snapMode: this.debugSnapMode
    });
    const dragSession = this.dragSession;
    const firstId = dragSession.selectedIds[0] || "";
    const device = firstId ? this.scene.getDevice(firstId) : null;
    const startPosition = firstId ? dragSession.startPositions.get(firstId) : null;
    const offset = firstId ? dragSession.offsetMap().get(firstId) : null;
    const snapDebug = dragSession.snapDiagnostics?.debug || {};
    const guides = dragSession.snapGuides || null;
    const predictedWorld = startPosition && offset
      ? { x: startPosition.x + offset.dx, y: startPosition.y + offset.dy }
      : null;
    const predictedScreen = predictedWorld ? worldToScreen(this.camera, predictedWorld) : null;
    const measure = guides?.measure || null;
    const bestX = snapDebug.bestX || null;
    const bestY = snapDebug.bestY || null;
    const entry = {
      frame: ++this.snapTraceFrame,
      ...
      pointer: {
        ...
        canvasX: Number(point?.x || 0),
        canvasY: Number(point?.y || 0),
        worldX: Number(worldPoint?.x || 0),
        worldY: Number(worldPoint?.y || 0)
      },
      dragStart: {
        clientX: dragSession.startClient?.clientX ?? null,
        clientY: dragSession.startClient?.clientY ?? null,
        worldX: Number(dragSession.startWorld?.x || 0),
        worldY: Number(dragSession.startWorld?.y || 0)
      },
      drag: {
        rawDx: Number(dragSession.rawDx || 0),
        rawDy: Number(dragSession.rawDy || 0),
        axisLock: dragSession.axisLock || null,
        preSnapDx: Number(dragSession.preSnapDx || 0),
        preSnapDy: Number(dragSession.preSnapDy || 0)
      },
      snap: {
        enabled: this.objectSnappingEnabled(),
        mode: this.debugSnapMode,
        candidateCount: Number(dragSession.snapCandidateCount || 0),
        targetCount: Number(dragSession.snapTargetCount || 0),
        xActive: Boolean(bestX),
        yActive: Boolean(bestY),
        xSource: bestX?.source || null,
        ySource: bestY?.source || null,
        xTargetId: bestX?.targetId || null,
        yTargetId: bestY?.targetId || null,
        xDiff: bestX?.diff ?? null,
        yDiff: bestY?.diff ?? null,
        correctionX: Number(snapDebug.correctionX || 0),
      ...
      result: {
        finalDx: Number(dragSession.dx || 0),
        finalDy: Number(dragSession.dy || 0)
      },
      render: {
        selectedId: firstId,
        offsetDx: Number(offset?.dx || 0),
        offsetDy: Number(offset?.dy || 0),
        objectWorldX: Number(device?.x || startPosition?.x || 0),
        objectWorldY: Number(device?.y || startPosition?.y || 0),
        predictedRenderedWorldX: predictedWorld?.x ?? null,
        predictedRenderedWorldY: predictedWorld?.y ?? null,
        predictedRenderedScreenX: predictedScreen?.x ?? null,
        predictedRenderedScreenY: predictedScreen?.y ?? null
      },
      guides: {
    ...
    trace.push(entry);
    this.hud?.setMetric("snap trace", `${trace.frames.length}/${trace.max} frames / ${this.debugSnapMode}`);
    return entry;
  }
  ...
  objectSnappingEnabled() {
    if (this.debugSnapMode === "off") return false;
  ...
  objectSnapHudText(dragSession = this.dragSession) {
    ...
    if (this.debugObjectSnapping && debug) {
      return `${this.debugSnapMode} / ${snapped} / ${ready} / raw ${formatSnapNumber(debug.rawDx)},${formatSnapNumber(debug.rawDy)} -> ${formatSnapNumber(debug.finalDx)},${formatSnapNumber(debug.finalDy)} / ${dragSession.snapCandidateCount}/${dragSession.snapTargetCount} candidates / ${source} / ${dragSession.snapMs.toFixed(3)} ms`;
    }
  ...
  updateObjectSnapDebugHud() {
    ...
    this.hud.setMetric("snap mode", this.debugSnapMode);
    if (!debug) {
  ...
  restoreEngineSelection(selection = {}) {
    ...
    (selection.racks || []).forEach(rackId => {
      ...
      if (!rack) return;
      this.scene.selectedRackIds.add(rack.id);
      this.scene.rackChildIds(rack.id).forEach(childId => {
        if (this.scene.devicesById.has(childId)) this.scene.selectedIds.add(childId);
      });
    });
    ...
    if (selection.jumpLinkId && this.scene.getJumpLink(selection.jumpLinkId)) {
      this.scene.selectJumpLinkOnly(selection.jumpLinkId);
    }
  }
  ...
  completeMarquee() {
    ...
    const ids = uniqueItems(this.scene.spatialIndex.queryRect(rect)
      ...
      .filter(Boolean));
    const rackFrameIds = uniqueItems((this.scene.rackIndex?.queryRect?.(rect) || [])
      .map(item => item.payload?.rack?.id || item.rack?.id || "")
      .filter(Boolean));
    const rackIds = uniqueItems([
      ...rackFrameIds,
      ...ids.map(id => this.scene.deviceRackId(id)).filter(rackId => this.scene.getRack(rackId))
    ]);
    const expandedIds = uniqueItems([
    ...
    if (state.additive) {
      ...
      rackIds.forEach(rackId => {
        if (selectedBefore.has(rackId)) this.scene.selectedRackIds.delete(rackId);
        else this.scene.selectedRackIds.add(rackId);
      });
    } else {
      this.scene.selectMany(expandedIds);
      rackIds.forEach(rackId => this.scene.selectedRackIds.add(rackId));
    }
    this.hud.setMetric("marquee", `${expandedIds.length} object${expandedIds.length === 1 ? "" : "s"} / ${rackIds.length} rack${rackIds.length === 1 ? "" : "s"}${state.additive ? " toggled" : ""}`);
    this.updateSelectionHud();
  ...
  cancelActiveInteraction(reason = "cancelled", { updateHud = true } = {}) {
    ...
    this.cancelPendingJumpPress(reason, { updateHud: false });
    this.jumpLinkCreate = null;
    if (this.commentBoxDrag) {
  ...
  hasSelection() {
    return Boolean(
      ...
      || this.scene.selectedRackIds.size
  ...
  contextMenuTarget(event) {
    ...
    const rackHit = hitTestRack(this.scene, world);
    if (rackHit.rack) {
      this.recordRackCanvasDiagnostic("context rack frame", {
        rackId: rackHit.rack.id,
        chosenAction: "open rack context menu",
        hitCandidates: rackHit.candidates
      });
      return {
        type: "rack",
  ...
  contextMenuObjectTarget(device) {
    ...
    const rack = rackId ? this.scene.getRack(rackId) : null;
    if (rack) {
      this.recordRackCanvasDiagnostic("context child device", {
        engineChildId: device.id,
        productionChildId: device.sourceId || device.id,
        rackId,
        chosenAction: "open rack context menu"
      });
      return {
  ...
  jumpLinkPreviewState() {
    ...
    const toColor = compatibility.valid
      ...
      : "rgba(119,132,146,.72)";
    const points = jumpLinkBezierPolyline(this.jumpLinkCreate.fromPoint, targetPoint);
    return {
  ...
  visibleJumpLinkOverlays() {
    ...
    const addLink = (link, mode) => {
      ...
      const from = this.jumpNodeCenter(outputDevice);
      const to = this.jumpNodeCenter(inputDevice);
    ...
    if (this.scene.selectedJumpLinkId) {
      addLink(this.scene.getJumpLink(this.scene.selectedJumpLinkId), "link-selected");
    }
    [...this.scene.selectedIds].forEach(id => {
      const device = this.scene.getDevice(id);
      if (!isJumpNodeDevice(device)) return;
      addLink(this.scene.jumpLinkForNode(id), "pair-selected");
    });
    const hoveredDevice = this.hoverState.device?.device || this.hoverState.device || this.hoverState.connector?.device || null;
  ...
  hitTestVisibleJumpLink(worldPoint, screenTolerancePx = 8) {
    const start = performance.now();
    const tolerance = this.hitToleranceWorld(screenTolerancePx);
    let best = null;
    let candidates = 0;
    this.visibleJumpLinkOverlays().forEach(overlay => {
      const link = overlay?.id ? this.scene.getJumpLink(overlay.id) : null;
      ...
      candidates += 1;
      const hit = HitTest.distanceToPolyline(points, worldPoint);
      if (hit.distance <= tolerance && (!best || hit.distance < best.distance)) {
        best = { link, overlay, points, distance: hit.distance, point: hit.point, segmentIndex: hit.segmentIndex };
      }
    });
    return {
      link: best?.link || null,
      overlay: best?.overlay || null,
      points: best?.points || [],
  ...
  updateSelectionHud() {
    ...
    const selectedRacks = this.scene.selectedRackIds.size;
    ...
    this.hud.setMetric("selected jump links", this.scene.selectedJumpLinkId || "-");
  ...
  renderEngineInspector() {
    ...
    const selectedConnectors = [...this.scene.selectedConnectorKeys]
      ...
      .filter(Boolean);
    const selectedJumpLink = this.scene.selectedJumpLinkId ? this.scene.getJumpLink(this.scene.selectedJumpLinkId) : null;
    if (selectedJumpLink && !selectedRacks.length && !selectedDevices.length && !selectedWires.length && !selectedRoutePoints.length && !selectedConnectors.length) {
      const output = this.scene.getDevice(selectedJumpLink.outputJumpId);
      const input = this.scene.getDevice(selectedJumpLink.inputJumpId);
      this.inspectorPanel.innerHTML = `
        <h3>Engine Inspector</h3>
        ${detailsMarkup([
          ["Type", "Jump Link"],
          ["Link ID", selectedJumpLink.id],
          ["Output", output ? output.label || output.id : selectedJumpLink.outputJumpId],
    ...
    if (selectedWires.length === 1 && !selectedRacks.length && !selectedDevices.length && !selectedRoutePoints.length && !selectedConnectors.length) {
    ...
    if (selectedConnectors.length === 1 && !selectedRacks.length && !selectedDevices.length && !selectedWires.length && !selectedRoutePoints.length) {
  ...
  createSnapDebugScene() {
    if (!this.debugObjectSnapping || !this.ready) return false;
    const rect = this.canvasClientRect();
    if (!rect) return false;
    const width = 260;
    const height = 170;
    const gap = 320;
    const anchor = this.clientPointToWorld(
      rect.left + rect.width * 0.36,
      rect.top + rect.height * 0.36
    ) || { x: this.camera.x + 180 / Math.max(this.camera.zoom, 0.001), y: this.camera.y + 180 / Math.max(this.camera.zoom, 0.001) };
    const idSuffix = Math.round(performance.now()).toString(36);
    const template = {
      id: `debug-snap-template-${idSuffix}`,
      name: "Snap Test Device",
      model: "SNAP-RECT",
      category: "Matrixes",
      width,
      height,
    ...
    const first = {
      instanceId: `debug-snap-a-${idSuffix}`,
      templateId: template.id,
      templateOverride: deepClone(template),
      name: "Snap A",
    ...
    const second = {
      instanceId: `debug-snap-b-${idSuffix}`,
      templateId: template.id,
      templateOverride: deepClone(template),
      name: "Snap B",
      x: first.x + gap,
    ...
    const created = this.createDevicesFromLibraryDrop([first, second]);
    if (!created) return false;
    this.scene.selectOnly(first.instanceId);
    this.updateSelectionHud();
    this.renderEngineInspector();
    this.scheduleRender();
    const centerWorld = {
    ...
    const centerScreen = worldToScreen(this.camera, centerWorld);
    writeSnapTestSceneDomSnapshot({
      mode: this.debugSnapMode,
      firstId: first.instanceId,
      secondId: second.instanceId,
      firstRect: { x: first.x, y: first.y, width, height },
      secondRect: { x: second.x, y: second.y, width, height },
      firstCenterWorld: centerWorld,
      firstCenterScreen: {
    ...
    this.hud?.setMetric("snap test scene", `${first.instanceId} / ${second.instanceId}`);
    return true;
  }
  ...
  deleteSelectedJumpLink() {
    const linkId = this.scene.selectedJumpLinkId || "";
    const link = linkId ? this.scene.getJumpLink(linkId) : null;
    if (!link) {
      this.scene.clearSelectedJumpLink?.();
      this.updateSelectionHud();
      this.scheduleRender();
    ...
    this.beginProductionCommit("delete jump link");
    ...
    this.scene.clearSelectedJumpLink?.();
    this.markCommitted("delete jump link", result.mutationMs || 0, {
      jumpLinkDelete: true,
      jumpLinkId: result.linkData.id
    });
    this.updateSelectionHud();
    this.updateInteractionHud("jump-link-delete");
    return true;
  }

  disconnectSelectedJumpNodes() {
    const selectedLink = this.scene.selectedJumpLinkId ? this.scene.getJumpLink(this.scene.selectedJumpLinkId) : null;
    const primaryId = this.scene.primarySelectedJumpId
      || selectedLink?.outputJumpId
      || [...this.scene.selectedIds].find(id => isJumpNodeDevice(this.scene.getDevice(id)))
      || "";
    const link = selectedLink || (primaryId ? this.scene.jumpLinkForNode(primaryId) : null);
    if (!link) return false;
    this.beginProductionCommit("disconnect jump nodes");
    const result = this.removeJumpLink(link.id);
    if (!result.linkData) return false;
    this.recordCommand(deleteJumpLinkCommand(result.linkData, result.index));
    if (selectedLink) this.scene.clearSelection();
    else this.scene.selectJumpPairPrimary(primaryId);
  ...
  selectedRackIds() {
    const ids = new Set([...(this.scene.selectedRackIds || [])]);
  ...
  deleteSelectedDevices(deviceIds = this.selectedDeletableDeviceIds()) {
    ...
    const explicitRackIds = [...(this.scene.selectedRackIds || [])]
      .filter(rackId => this.scene.getRack(rackId));
    if (explicitRackIds.length) {
      this.deleteSelectedRackInstances(explicitRackIds);
      return;
    }
  ...
  runSceneValidation() {
    ...
    if (this.dragSession || this.pendingDrag || this.pendingJumpPress || this.jumpLinkCreate || this.routePointDrag || this.wireSegmentDrag || this.wireCreate || this.marqueeState) {
  ...
  scheduleRender() {
    ...
    this.renderFrame = requestAnimationFrame(() => {
      ...
      const renderMs = this.renderer.draw(this.scene, this.camera, {
        selectedIds: this.scene.selectedIds,
        selectedRackIds: this.scene.selectedRackIds,
        selectedWireIds: this.scene.selectedWireIds,
      ...
      this.hud.setMetric("rack frames", `${frameStats.rackFrames || 0} / ${(frameStats.rackFrameMs || 0).toFixed(2)} ms`);
      ...
      this.hud.setMetric("rack labels", `${labelStats.racks || 0}`);
...
function engineDebugSnapMode() {
  const value = String(new URLSearchParams(window.location.search).get("debugSnapMode") || "full").toLowerCase();
...
function offsetDebugRect(rect, dx = 0, dy = 0) {
  if (!rect) return null;
  return {
...
function worldToScreen(camera, point) {
  return {
    x: (Number(point?.x || 0) - Number(camera?.x || 0)) * Number(camera?.zoom || 1),
    y: (Number(point?.y || 0) - Number(camera?.y || 0)) * Number(camera?.zoom || 1)
  };
}

function ensureSnapTraceBuffer(meta = {}) {
  if (typeof window === "undefined") return null;
  const max = 500;
  const existing = window.__AVD_SNAP_TRACE__;
  if (existing && Array.isArray(existing.frames) && typeof existing.push === "function") {
    existing.max = max;
    existing.meta = { ...(existing.meta || {}), ...meta };
    writeSnapTraceDomSnapshot(existing);
    return existing;
  }
  const buffer = {
    max,
    ...
    push(entry) {
      ...
      if (this.frames.length > this.max) this.frames.splice(0, this.frames.length - this.max);
      window.__AVD_SNAP_LAST__ = entry;
      writeSnapTraceDomSnapshot(this);
      return entry;
    },
    clear() {
      this.frames.length = 0;
      window.__AVD_SNAP_LAST__ = null;
      writeSnapTraceDomSnapshot(this);
    },
    last(count = 1) {
      return this.frames.slice(Math.max(0, this.frames.length - Math.max(1, Number(count) || 1)));
    }
  };
  window.__AVD_SNAP_TRACE__ = buffer;
  window.__AVD_SNAP_LAST__ = null;
  writeSnapTraceDomSnapshot(buffer);
  return buffer;
}

function writeSnapTraceDomSnapshot(buffer) {
  if (typeof document === "undefined" || !buffer) return;
  let element = document.getElementById("engineSnapTraceDump");
  if (!element) {
    element = document.createElement("script");
    element.id = "engineSnapTraceDump";
  ...
  element.textContent = JSON.stringify({
    ...
    last: window.__AVD_SNAP_LAST__ || null
  });
}

function writeSnapTestSceneDomSnapshot(snapshot) {
  if (typeof document === "undefined" || !snapshot) return;
  let element = document.getElementById("engineSnapTestSceneDump");
  if (!element) {
    element = document.createElement("script");
    element.id = "engineSnapTestSceneDump";
    element.type = "application/json";
    element.dataset.debugOnly = "true";
    document.body.appendChild(element);
  }
...
function captureEngineSelection(scene) {
  return {
    ...
    jumpLinkId: scene?.selectedJumpLinkId || ""
  };
}
