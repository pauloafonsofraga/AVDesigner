...
export function createSharedBusRenderingAPI() {
  function sharedBusOrthogonalSegments(layout, body, { nodeInset = 10, offsetX = 0, offsetY = 0 } = {}) {
    ...
    const centerY = layout.centerY ?? (minY + maxY) / 2;
    const requestedJunction = layout.fieldJunctionX ?? averageX + direction * 26;
    if (!Number.isFinite(centerY) || centerY < minY || centerY > maxY || !Number.isFinite(requestedJunction)) return null;
    const junction = Math.max(left + margin, Math.min(right - margin, requestedJunction));
    // Keep the vertical trunk clear of the field border; only the central stem
    // bridges that clearance. A body too narrow for the node corridor is not drawn.
    ...
    return {
      ...
      stem: { x1: trunkX, y1: centerY + offsetY, x2: junction + offsetX, y2: centerY + offsetY },
