...
thumbnail crop, transient placement previews, and output/report/viewer clones
remain intentionally outside that migration.
...
Iteration 53.4 completes the Engine-mode preview migration. Rack Builder,
Device Editor, Node Builder Canvas Appearance, and Title Block previews now use
the same SceneGraph/WebGL renderer and shared visual helpers as placed Engine
objects for committed production artwork; SVG is reserved for Legacy fallback,
authoring overlays, crop controls, transient previews, and output paths.
...
Iteration 41 kept the modular Device Editor, Cards tab, the earlier dedicated
card-slot panel, drag/drop card assignment, and per-slot node-field editing in
the production DOM modal. Current builds keep reusable card definitions in
Cards and moved card-slot creation, installation, replacement, movement, and
removal into the Connectors tab.
...
| Faceplate image rendering | Custom PNG faceplates draw at editable size/position; deleted faceplate removes the space; generated PD faceplates use SVG plug assets. | Engine metadata carries faceplate state and draws real faceplate images into cached textures with Legacy-style placement and async affected-texture invalidation. Engine-mode Device Editor preview uses the same shared Engine texture path. | partial | `faceplateHeightForTemplate`, `renderDeviceEditorPreview`, `drawDeviceBody` at `8301fbf:index.html:33926`, `40081`, `43703` | `normalizeVisualMetadata` in `src/engine/sceneGraph.js`; `drawFaceplate`, `legacyFaceImagePlacement`, image cache helpers in `src/engine/deviceVisualBuilder.js`; asset invalidation in `src/engine/textureCache.js`; `EnginePreviewSurface` in `src/engine/enginePreview.js` | `faceImage`, `faceImageNaturalWidth`, `faceImageNaturalHeight`, `faceImageScale`, `faceImageScaleX`, `faceImageScaleY`, `faceImageOffsetX`, `faceImageOffsetY`, `faceplateDeleted` | High: real image loading into texture atlas/canvas is required. | Faceplate editor controls remain DOM/Legacy; production preview artwork is Engine-owned in Engine mode. | Visual edits need texture invalidation only for changed device/template. | Existing fields preserved. | PDF/viewer may still diverge until output path shares helpers. | high | 53.4 | PD generated faceplates and standard PNG faceplates now share the Engine preview/canvas visual path in Engine mode. |
...
| Device Editor modal | Multi-tab Legacy modal edits device, faceplate, connectors, cards, slots, defaults, tech specs. | Modal controls remain production/DOM UI, while Engine-mode committed preview artwork is rendered by `EnginePreviewSurface`; Apply records one targeted command and refreshes affected scene devices/wires. | partial | `renderDeviceEditorPreview`, editor handlers near `8301fbf:index.html:40081` | `EnginePreviewSurface` and `createPreviewDeviceFromDraft` in `src/engine/enginePreview.js`; `applyDeviceEditor` in `index.html`; `commitDeviceEditorApplyFromProduction` in `src/engine/productionBridge.js`; project normalization in `src/engine/projectAdapter.js` | templates, connectors, cards, slots, faceplate metadata, tech specs | High only after Apply, because visual cache must invalidate changed templates/instances. | DOM editor remains complete authoring source of truth; preview production artwork is Engine-owned in Engine mode. | Apply is now one Engine command with snapshot undo/redo. | Existing save format unchanged. | Reports consume updated production data. | medium | 53.4 | Do not rebuild the editor forms in WebGL; persistent preview artwork now shares the main canvas renderer. |
...
| Rack Builder | Separate modal with device source pane, rack list, internal wiring, exposed rack ports, preview pan/zoom. | Production Rack Builder remains source of truth; Engine-mode preview renders committed rack visuals through the shared preview surface; authoring controls stay as SVG overlay. | partial | `renderRackBuilder`, `renderRackBuilderPreview`, Rack Builder overlay handlers in `index.html` | `src/engine/rackPreview.js`, `EnginePreviewSurface` in `src/engine/enginePreview.js`, Rack objects normalized by `sceneGraph.js`; device visuals by `deviceVisualBuilder.js` | rack definitions, stable `sourceRackDeviceId`, internal wires, exposed ports, child device transforms | Medium: preview and placed rack now share the rendering path; authoring behavior remains production-side. | Builder UI remains production/DOM-owned; committed visuals are Engine. | Pan/zoom/Fit are camera-only; child drag and route edits update affected Engine preview items. | Existing data preserved. | Reports/export read rack/project data. | medium | 53.4 | Legacy `?legacy=1` still uses old SVG Rack Builder preview; Engine-mode committed rack visuals no longer redraw through SVG. |
