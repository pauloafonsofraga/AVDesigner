...
Iteration 42 restores the Project Custom Devices workflow as a project-template
workflow. The Device Editor remains Legacy/DOM-owned, but Create New Device,
duplicate-to-custom, edit, delete, search/list, thumbnail refresh, and
drag/drop now target project custom templates. Applying a custom-template edit
increments a visual revision for future Engine drops and texture-cache
invalidation. Already placed instances remain unchanged by design.
...
| Create, duplicate, and delete device template workflow | Buttons in Device tab plus library context menu create/duplicate/delete templates. | Create New Device opens a new Project Custom template draft. Built-in duplicates are copied into Project Custom Devices. Project custom duplicates get unique template/connector IDs, and delete is confirmed/safe. | delegated | Device tab handlers in `index.html`; `showLibraryDeviceContextMenu` at `8301fbf:index.html:48259` | `openDeviceEditorWithNewDevice`, `duplicateDeviceTemplateForCollection`, `deleteProjectCustomDeviceTemplate` in `index.html`; `ProductionEngineBridge.refreshSceneFromProduction` for canvas sync | template IDs, copied connectors/cards/visuals, `projectCustomDevice`, `visualRevision` | Medium when duplicated template shares image/card metadata. | Legacy UI owns confirmation and forms. | Template-level undo remains production-side; placed-device creation is Engine undoable. | Compatible if IDs remain stable. | No direct output impact. | medium | 42 | Engine cache keys include project custom visual revisions so edited custom templates do not reuse stale future textures. |
