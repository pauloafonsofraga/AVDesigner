...
Iteration 39 is deliberately an audit pass. It does not migrate reports,
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Normal device body | SVG device body with dark shell, faceplate area, labels, connector rails, selected glow, and detailed child elements. | Cached bitmap texture drawn by Engine with simplified body, header, faceplate placeholder, connector markers, and labels. | partial | `drawDeviceBody` at `8301fbf:index.html:43703` | `buildDeviceVisual`, `drawDeviceVisual`, `drawDeviceHeader` in `src/engine/deviceVisualBuilder.js`; `drawDeviceLabel` in `src/engine/renderer.js` | `deviceInstances`, template `width`, `height`, `brand`, `name`, `model`, `category` | High: texture fidelity and cache invalidation define main canvas look. | Device selection and hover are Engine-owned; edit workflow is still Legacy modal. | Position moves are command based; visual-only body changes need texture invalidation. | Existing data is compatible. | Viewer/PDF still use existing output paths. | medium | 40 | The speed architecture is good; visual polish should happen through texture drawing, not DOM restoration. |
...
| Faceplate image rendering | Custom PNG faceplates draw at editable size/position; deleted faceplate removes the space; generated PD faceplates use SVG plug assets. | Engine metadata carries faceplate state, but texture currently uses simplified generated drawing and basename hints rather than full image fidelity. | partial | `faceplateHeightForTemplate`, `renderDeviceEditorPreview`, `drawDeviceBody` at `8301fbf:index.html:33926`, `40081`, `43703` | `normalizeVisualMetadata` in `src/engine/sceneGraph.js`; `drawFaceplate` in `src/engine/deviceVisualBuilder.js` | `faceImage`, `faceImageNaturalWidth`, `faceImageNaturalHeight`, `faceplateScaleX`, `faceplateScaleY`, `faceplateDeleted` | High: real image loading into texture atlas/canvas is required. | Faceplate editor remains Legacy. | Visual edits need texture invalidation only for changed device/template. | Existing fields preserved. | PDF/viewer may still diverge until output path shares helpers. | high | 42 | This is one of the biggest visual fidelity gaps for real projects. |
...
| Installed card visual captions | Card blocks have colored caption bars, slot/card names, custom backgrounds, and generated node fields. | Engine draws simplified dashed card areas and captions; field rendering is not full Legacy. | partial | `renderDeviceEditorPreview`, `drawDeviceBody`, card slot helpers | `drawCardAreas` in `src/engine/deviceVisualBuilder.js`; card metadata from `sceneGraph.js` | card visual color, caption color/background, slot ID, installed card type | High for E2-style devices. | Card editing remains Legacy. | Texture invalidation only when card visual/slot changes. | Compatible. | Viewer/PDF may not match Engine yet. | high | 41 | Card labels should remain white with black outline where Legacy requires it. |
...
| Adapter/breakout visual | Transparent/dashed compact device with name outside, no node fields, internal wiring, multi-connection breakout rules. | Engine supports adapter kind visually and interaction, but internal gradient wiring/output parity remains partial. | partial | `isAdapterTemplate`, `drawDeviceBody` at Legacy commit; adapter code in production `index.html` | `drawAdapterVisual` in `src/engine/deviceVisualBuilder.js`; adapter normalization in `projectAdapter.js` | `isAdapterBreakout`, compact dimensions, connectors, internal wires | Medium: simple but visually distinctive. | Device Editor toggle remains production UI. | Multi-connection and internal route changes need command audit. | Compatible. | PDF/export adapter styling has had regressions and needs later output pass. | medium | 45 | Engine should keep adapter fast and compact, not reuse full device body. |
| Adapter internal gradient wiring | Internal cable fades from input color to output color and supports breakout fan-out. | Not fully represented as separate Engine visual system. | missing | Legacy adapter drawing in `drawDeviceBody` and production helper blocks | No dedicated Engine helper beyond `drawAdapterVisual` | internal adapter connection records if present, connector colors | Medium. | No current Engine editing UI. | Needs undo only if user can edit internal mapping. | Save format must preserve internal links. | Reports count adapter device and external cables. | medium | 45 | Needs a small visual helper, not a full Rack Builder. |
