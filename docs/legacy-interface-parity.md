...
| Create, duplicate, and delete device template workflow | Buttons in Device tab plus library context menu create/duplicate/delete normal templates; placed-device context duplicate feeds the project custom list. | Create New Device opens a main-library draft. Library context duplicate creates another main-library template. Canvas **Duplicate Device** creates the project custom template copy from the placed instance's effective data. Project custom duplicates get unique template IDs and delete is confirmed/safe. | delegated | `openDeviceEditorWithNewDevice`, `renderProjectCustomDevices`, `startProjectCustomDeviceDrag`, `showDeviceContextMenu`, `duplicateLibraryDevice` at `8301fbf:index.html:35886`, `42037`, `42088`, `48082`, `48298` | `openDeviceEditorWithNewDevice`, `createProjectCustomDeviceFromCanvasInstance`, `duplicateDeviceTemplateForCollection`, `deleteProjectCustomDeviceTemplate` in `index.html`; `ProductionEngineBridge.refreshSceneFromProduction` for canvas sync | master template IDs, copied connectors/cards/visuals, `projectCustomDevice`, `visualRevision` | Medium when duplicated template shares image/card metadata. | Legacy UI owns confirmation and forms. | Template-level undo remains production-side; placed-device creation is Engine undoable. | Compatible if IDs remain stable. | No direct output impact. | medium | 42.1 | Master and Project Custom entries share the saved project library array but are treated as separate collections through flags and helper filters. |
