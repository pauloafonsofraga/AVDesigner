...
Iteration 40.1 is the first implementation pass after the Iteration 39 audit. It
restores modular chassis texture fidelity for installed-card devices by carrying
installed card metadata, caption colors, card lane geometry, and per-slot
connector field values through the Engine normalization path into cached device
textures. It also draws real faceplate images in the Engine texture cache and
rebuilds only affected device textures when those image assets finish loading.
