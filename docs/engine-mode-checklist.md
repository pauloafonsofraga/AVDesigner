...
`iteration51-7-info-field-hover-v17`.
...
Iteration 51.7 keeps the Iteration 51.6 zoom-detail model and fixes connector
information-field hover ownership. Normal source field rectangles stay active
while a magnified preview is visible, the preview is anchored from the source
field's screen-space centre, and `debugZoomDetail=1` now reports source rect,
...
keyboard shortcuts remain ordinary DOM UI; Engine continues to own canvas
editing, scene commands, and renderer state. Use `debugShell=1` to show the
Shell Debug panel and forward the same shell state into the Engine HUD. Shell
panel resizing, tab changes, menus, tooltips, and toolbar actions must not
trigger full Engine scene rebuilds.

Iteration 49 restores the Legacy Matrix Routing workflow in the Engine Editor.
Matrix-capable devices still use the Legacy `template.isMatrixRouter` flag,
eligible input/output ports are derived from the shared connector eligibility
rules, and routes remain serialized as
`instance.matrixRoutes = { [outputConnectorId]: inputConnectorId }`. The DOM
Matrix Routing modal is intentionally retained, but in Engine mode its select
menus and crosspoint cells dispatch `MatrixRoutingCommand`s through the Engine
bridge. A matrix edit updates production data, undo/redo state, the inspector,
and the open modal without rebuilding device textures or project wires. Use
`debugMatrix=1` with the Engine HUD to inspect matrix device ID, route counts,
invalid routes, modal render count, last command, and the full-scene rebuild
guardrail.
...
devices, a Legacy blue frame, explicit rack selection, rack hit targets, rack
context menus, rack drag/delete commands, and definition-device to
canvas-instance mapping. External cables still connect to the real child-device
connectors. Rack-internal connections stay stored on rack definitions, render
as non-selectable derived Engine wires when `showInternalWiring` is enabled,
and are not duplicated into normal project cables. Use `debugRackBuilder=1`
with the Engine HUD to inspect rack counts, bounds, child-device grouping,
selected rack IDs, and rack scene refreshes.
...
drag delta are recalculated from the current pointer position on every drag
frame. Dashed alignment guides are drawn only along the moving edge that is
...
Iteration 51.7 keeps `debugZoomDetail=1` as connector zoom diagnostics only.
Hovering connector info fields must not rebuild device textures, mutate project
...
2. Open the default engine editor with cache busting:
   `index.html?v=iteration51-7-info-field-hover-v17`.
3. Open the explicit engine editor: `index.html?engine=1&v=iteration51-7-info-field-hover-v17`.
4. Open the snapping debug HUD:
   `index.html?engine=1&debugObjectSnapping=1&v=iteration51-7-info-field-hover-v17`.
5. Open the compatibility default-test alias:
   `index.html?engineDefaultTest=1&v=iteration51-7-info-field-hover-v17`.
6. Open the legacy editor fallback:
   `index.html?legacy=1&v=iteration51-7-info-field-hover-v17`.
7. Open the alternate legacy fallback:
   `index.html?engine=0&v=iteration51-7-info-field-hover-v17`.
8. Open the debug loading guard:
   `index.html?engine=1&debugLoad=1&v=iteration51-7-info-field-hover-v17`.
9. Open a timed loading guard:
   `index.html?engine=1&loadDelay=1500&v=iteration51-7-info-field-hover-v17`.
10. Open the expanded engine HUD:
   `index.html?engine=1&debugHud=1&v=iteration51-7-info-field-hover-v17`.
11. Open shell diagnostics with the Engine HUD:
   `index.html?engine=1&debugHud=1&debugShell=1&v=iteration51-7-info-field-hover-v17`.
12. Open shell diagnostics in Legacy fallback:
   `index.html?legacy=1&debugShell=1&v=iteration51-7-info-field-hover-v17`.
13. Open output diagnostics in the default engine editor:
   `index.html?debugOutput=1&v=iteration51-7-info-field-hover-v17`.
14. Open output diagnostics in Legacy fallback:
   `index.html?legacy=1&debugOutput=1&v=iteration51-7-info-field-hover-v17`.
15. Open the device visual diagnostic HUD/layer view:
   `index.html?engine=1&debugDeviceVisual=1&v=iteration51-7-info-field-hover-v17`.
16. Open the Power Distro diagnostic HUD:
   `index.html?engine=1&debugHud=1&debugPowerDistro=1&v=iteration51-7-info-field-hover-v17`.
17. Open the Rack Builder diagnostic HUD:
   `index.html?engine=1&debugHud=1&debugRackBuilder=1&v=iteration51-7-info-field-hover-v17`.
18. Open the Matrix Routing diagnostic HUD:
   `index.html?engine=1&debugHud=1&debugMatrix=1&v=iteration51-7-info-field-hover-v17`.
19. Open the Device Library drag/drop debug overlay:
   `index.html?debugLibraryDrag=1&v=iteration51-7-info-field-hover-v17`.
20. Open the explicit Engine drag/drop debug overlay:
   `index.html?engine=1&debugLibraryDrag=1&v=iteration51-7-info-field-hover-v17`.
21. Open the Legacy drag/drop debug overlay:
   `index.html?legacy=1&debugLibraryDrag=1&v=iteration51-7-info-field-hover-v17`.
22. Open compatibility diagnostics while drawing wires:
    `index.html?engine=1&debugCompatibility=1&v=iteration51-7-info-field-hover-v17`.
23. Open routing diagnostics while selecting or editing orthogonal wires:
    `index.html?engine=1&debugHud=1&debugRouting=1&v=iteration51-7-info-field-hover-v17`.
24. Open endpoint-rewire diagnostics:
    `index.html?engine=1&debugRewire=1&debugRouting=1&v=iteration51-7-info-field-hover-v17`.
25. Open the Project Custom identity overlay:
    `index.html?engine=1&debugCustomIdentity=1&v=iteration51-7-info-field-hover-v17`.
26. Open connector zoom-detail diagnostics:
    `index.html?engine=1&debugHud=1&debugZoomDetail=1&v=iteration51-7-info-field-hover-v17`.
...
Use `index.html?engine=1&debugHud=1&debugShell=1&v=iteration50` for the main
shell test. Confirm the Shell Debug panel and the Engine HUD both report:

- active tool
- focused element
- open modal/menu/context/tooltip
- left/right panel widths and collapse state
- active Device Library/Device Editor/Node Builder/Rack Builder tabs
- canvas CSS size and WebGL buffer size
- DPR, zoom, grid, snap, dirty, undo, redo
- shell listener count, resize count, last editor action, shortcut, and panel action
- shell full-scene rebuild count remains `0` for toolbar, panel, menu, and tab changes

Minimum shell checks:

1. Click every top toolbar button that does not open a file picker.
2. Toggle grid/snap/wire mode/hops/dark mode/voltage.
3. Open Device Editor, Node Builder, Rack Builder, Report, Publish, and Matrix Routing where available.
4. Resize the left panel/device tree and verify the debug panel updates without an Engine full-scene rebuild.
5. Change Device Editor tabs and Rack Builder selection; the active-tab diagnostics should update.
6. Press Escape to close menus/modals; no menu should stay stranded.
7. Verify `index.html?legacy=1&debugShell=1&v=iteration50` shows `Iteration 50 — Legacy Editor — iteration50`.

## Iteration 48.2 Rack Builder Checks

1. Open `index.html?engine=1&debugHud=1&debugRackBuilder=1&v=iteration50`.
...
3. Click any placed child device; the whole placed rack should select as one
   group and the debug HUD should show the selected rack ID.
4. Move the selected rack; every child device, external endpoint, and visible
   internal rack wire should move together.
...
10. Disable engine temporarily by using `legacy=1` or `engine=0`.

## Iteration 49 Matrix Routing Checks

1. Open `index.html?engine=1&debugHud=1&debugMatrix=1&v=iteration50`.
2. Load or place a device whose template has `isMatrixRouter: true`, such as an
   E2-style matrix/router device.
3. Right-click the device and choose **Edit Matrix Routing**.
4. Change an output select and click a crosspoint cell. The active cell should
   switch immediately, the modal should keep its scroll/focus, and the HUD
   `matrix full rebuilds` metric should remain `0`.
5. Undo and redo with the modal still open. The grid and route table should
   update immediately and the viewport should not move.
6. Save, reload, and reopen Matrix Routing. Routes should persist by connector
   IDs, and connector renames should only change labels.
7. Open `index.html?legacy=1&v=iteration50` with the same saved file and confirm
   the Legacy modal reads the same `matrixRoutes`.
8. Confirm no normal canvas wires are created by matrix route changes.
9. Disable engine temporarily by using `legacy=1` or `engine=0`.

## Iteration 42.4 Workflow Checks
...
- **Canvas Duplicate Device -> Project Custom Devices:** place a device on the
  ...
  source canvas device should remain unchanged, one shifted canvas copy should
  be inserted automatically, and one new searchable/editable/draggable entry
...
- **Custom-device diagnostics:** when needed, open
  `index.html?debugCustomDevices=1&v=iteration42-4` to show custom duplicate,
  drag-threshold, click-suppression, drop, and delete diagnostics.
- **Custom identity diagnostics:** open
  `index.html?engine=1&debugCustomIdentity=1&v=iteration42-4`, then perform the
  real browser workflow. Canvas **Duplicate Device** must add exactly one
  Project Custom template and one placed canvas instance. Dragging that Project
  Custom row to the canvas must add exactly one additional placed instance and
...
25. Compare Engine and Legacy connector behavior with the same project:
   `index.html?v=iteration42-4` beside
   `index.html?legacy=1&v=iteration42-4`.
25. In `index.html?engine=1&debugHud=1&v=iteration42-4`, confirm the HUD
...
38. Open `index.html?engine=1&debugHud=1&debugLayers=1&v=iteration40-10`, drag
...
49. Open `index.html?engine=1&debugHud=1&v=iteration40-10` and confirm the HUD
    rows `cable hops`, `cable hop calc`, `cable hop candidates`, and
    `cable hop dirty` update.
50. Open `index.html?engine=1&debugHud=1&debugRouting=1&v=iteration40-10`,
...
60. Repeat a Device Library drag/drop in `index.html?legacy=1&v=iteration40-10`
...
- Confirm the visible top-bar label shows either
  `Iteration 40.10 — Engine Editor — iteration40-10` or
  `Iteration 40.10 — Legacy Editor — iteration40-10`.
- Open `index.html?debugDeviceTexture=1&debugHud=1&v=iteration40-10`.
...
| LED surface visuals | Production draws LED/image surfaces with aspect-correct image rendering, virtual ports, and right-click image actions. | Engine now maps LED surfaces as `led-surface` canvas objects, renders image/placeholder content through cached textures, preserves virtual ports, and can sync `Use image size` / replacement from production. | partial | Browser smoke with LED PNG replacement/use-image-size plus scene validation. | medium | Viewer/PDF output remains production-owned. |
| image-object visuals | Production does not currently expose a standalone first-class image-object inspector. | Engine has a separate `image-object` scene kind for project data that contains image objects. | partial | Data normalization only unless project contains image objects. | medium | UI parity waits on a Legacy/DOM workflow for image objects. |
| area visuals | Production areas sit behind wires/devices with translucent fill and caption. | Engine renders `area` objects in a background texture pass before static wires and devices, with live selection handles. | partial | Browser smoke with area select/drag/resize/delete. | medium | Area inspector remains DOM-owned. |
| comment visuals | Production comments render a box, caption/body, leader arrow, hover/edit behavior, and resize handles. | Engine renders `comment` objects with box text and leader geometry, and syncs DOM edits back into one cached object. | partial | Browser smoke with comment select/drag/edit/delete. | medium | Full hover magnifier/editor parity remains future polish. |
| title-block visuals | Production title blocks use a structured grid, fields, logo, and proportional scale. | Engine renders `title-block` objects using the same field/grid/logo model and live selection handles. | partial | Browser smoke with title block resize/edit/save/reload. | medium | PDF/viewer output remains production-owned. |
