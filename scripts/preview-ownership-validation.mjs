...
const EXPECTED_APP_BUILD_ID = "iteration54-2-1-jump-node-interaction";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.2.1";'), "app iteration should be 54.2.1");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.2.1");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-2-1-jump-node-interaction-modules";'), "module cache key should match 54.2.1");
...
assert.ok(indexHtml.includes("Jump Node Interaction"), "app build label should name 54.2.1");
