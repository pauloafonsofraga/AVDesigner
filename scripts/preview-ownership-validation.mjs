...
const EXPECTED_APP_BUILD_ID = "iteration54-9-1-card-fan-out-edge-hardening";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.9.1";'), "app iteration should be 54.9.1");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.9.1");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-9-1-card-fan-out-edge-hardening-modules";'), "module cache key should match 54.9.1");
...
assert.ok(indexHtml.includes("Card Fan-Out Edge Hardening"), "app build label should name 54.9.1");
