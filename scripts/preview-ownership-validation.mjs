...
const EXPECTED_APP_BUILD_ID = "iteration54-3-16-faceplate-upload-label-cleanup";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.3.16";'), "app iteration should be 54.3.16");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.3.16");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-3-16-faceplate-upload-label-cleanup-modules";'), "module cache key should match 54.3.16");
...
assert.ok(indexHtml.includes("Faceplate Upload Label Cleanup"), "app build label should name 54.3.16");
