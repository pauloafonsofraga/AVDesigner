...
const EXPECTED_APP_BUILD_ID = "iteration54-31-0-read-only-engine-output-viewer";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.31.0";'), "app iteration should be 54.31.0");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.31.0");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-31-0-read-only-engine-output-viewer-modules";'), "module cache key should match 54.31.0");
...
assert.ok(indexHtml.includes("Read-Only Engine Output Viewer"), "app build label should name 54.31.0");
