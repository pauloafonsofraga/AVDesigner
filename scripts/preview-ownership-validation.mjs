...
const EXPECTED_APP_BUILD_ID = "iteration54-3-12-remove-processor-faceplate-tags";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.3.12";'), "app iteration should be 54.3.12");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.3.12");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-3-12-remove-processor-faceplate-tags-modules";'), "module cache key should match 54.3.12");
...
assert.ok(indexHtml.includes("Remove Processor Faceplate Tags"), "app build label should name 54.3.12");
