...
const EXPECTED_APP_BUILD_ID = "iteration54-33-0-engine-vector-pdf-output";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.33.0";'), "app iteration should be 54.33.0");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.33.0");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-33-0-engine-vector-pdf-output-modules";'), "module cache key should match 54.33.0");
...
assert.ok(indexHtml.includes("Engine Vector PDF Output"), "app build label should name 54.33.0");
