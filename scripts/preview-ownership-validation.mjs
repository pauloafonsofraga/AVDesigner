...
const EXPECTED_APP_BUILD_ID = "iteration54-23-1-side-aware-empty-node-append";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.23.1";'), "app iteration should be 54.23.1");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.23.1");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-23-1-side-aware-empty-node-append-modules";'), "module cache key should match 54.23.1");
...
assert.ok(indexHtml.includes("Side-Aware Empty Node Append"), "app build label should name 54.23.1");
