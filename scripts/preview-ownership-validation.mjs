...
const EXPECTED_APP_BUILD_ID = "iteration54-22-0-stable-faceplate-node-handoff";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.22.0";'), "app iteration should be 54.22.0");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.22.0");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-22-0-stable-faceplate-node-handoff-modules";'), "module cache key should match 54.22.0");
...
assert.ok(indexHtml.includes("Stable Faceplate Node Handoff"), "app build label should name 54.22.0");
