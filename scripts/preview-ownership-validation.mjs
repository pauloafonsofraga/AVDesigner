...
const EXPECTED_APP_BUILD_ID = "iteration54-9-0-atomic-card-definition-fan-out";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.9.0";'), "app iteration should be 54.9.0");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.9.0");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-9-0-atomic-card-definition-fan-out-modules";'), "module cache key should match 54.9.0");
...
assert.ok(indexHtml.includes("Atomic Card Definition Fan-Out"), "app build label should name 54.9.0");
