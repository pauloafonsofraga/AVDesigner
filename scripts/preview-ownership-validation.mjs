...
const EXPECTED_APP_BUILD_ID = "iteration54-3-2-connector-status-hit-targets";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.3.2";'), "app iteration should be 54.3.2");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.3.2");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-3-2-connector-status-hit-targets-modules";'), "module cache key should match 54.3.2");
...
assert.ok(indexHtml.includes("Connector Status Hit Targets"), "app build label should name 54.3.2");
