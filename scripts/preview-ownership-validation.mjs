...
const EXPECTED_APP_BUILD_ID = "iteration54-2-5-3-jump-link-play-cable";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.2.5.3";'), "app iteration should be 54.2.5.3");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.2.5.3");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-2-5-3-jump-link-play-cable-modules";'), "module cache key should match 54.2.5.3");
...
assert.ok(indexHtml.includes("Jump Link Play Cable"), "app build label should name 54.2.5.3");
