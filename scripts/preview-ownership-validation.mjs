...
const EXPECTED_APP_BUILD_ID = "iteration54-4-2-matrix-routing-route-columns";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.4.2";'), "app iteration should be 54.4.2");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.4.2");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-4-2-matrix-routing-route-columns-modules";'), "module cache key should match 54.4.2");
...
assert.ok(indexHtml.includes("Matrix Routing Route Columns"), "app build label should name 54.4.2");
