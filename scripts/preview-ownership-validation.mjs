...
const EXPECTED_APP_BUILD_ID = "iteration54-31-1-bidirectional-jump-node-support";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.31.1";'), "app iteration should be 54.31.1");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.31.1");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-31-1-bidirectional-jump-node-support-modules";'), "module cache key should match 54.31.1");
...
assert.ok(indexHtml.includes("Bidirectional Jump Node Support"), "app build label should name 54.31.1");
