...
const EXPECTED_APP_BUILD_ID = "iteration54-3-11-editor-preview-modifier-wheel-zoom";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.3.11";'), "app iteration should be 54.3.11");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.3.11");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-3-11-editor-preview-modifier-wheel-zoom-modules";'), "module cache key should match 54.3.11");
...
assert.ok(indexHtml.includes("Editor Preview Modifier Wheel Zoom"), "app build label should name 54.3.11");
...
assert.ok(editorWheelGate.includes("IS_APPLE_POINTER_PLATFORM ? event?.altKey : event?.ctrlKey"), "Editor preview wheel zoom should use Option on Apple and Ctrl elsewhere");
assert.ok(
