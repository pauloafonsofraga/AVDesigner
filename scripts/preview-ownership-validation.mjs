...
const EXPECTED_APP_BUILD_ID = "iteration54-19-1-card-layer-handoff-fix";
...
assert.ok(indexHtml.includes('const APP_ITERATION = "54.19.1";'), "app iteration should be 54.19.1");
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${EXPECTED_APP_BUILD_ID}";`), "app build id should match 54.19.1");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-19-1-card-layer-handoff-fix-modules";'), "module cache key should match 54.19.1");
...
assert.ok(indexHtml.includes("Card Layer Handoff Fix"), "app build label should name 54.19.1");
