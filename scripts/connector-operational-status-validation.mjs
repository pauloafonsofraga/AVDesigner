...
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-29-0-per-connection-led-surface-ordering-modules";'), "module cache key should bust 54.29.0 modules");
assert.ok(bridgeSource.includes('ENGINE_BRIDGE_VERSION = "iteration54-29-0-per-connection-led-surface-ordering"'), "bridge version should identify Per-Connection LED Surface Ordering");
assert.ok(bridgeSource.includes('ENGINE_BRIDGE_FEATURE_LABEL = "per-connection-led-surface-ordering"'), "bridge feature label should identify Per-Connection LED Surface Ordering");
