...
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${BUILD_ID}";`), "app build id should identify Iteration 54.2.5");
assert.ok(indexHtml.includes('const APP_ITERATION = "54.2.5";'), "visible iteration should be 54.2.5");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-2-5-jump-interaction-legacy-actions-modules";'), "module cache key should bust 54.2.5 modules");
assert.ok(bridgeSource.includes(`ENGINE_BRIDGE_VERSION = "${BUILD_ID}"`), "Engine bridge version should identify Iteration 54.2.5");
