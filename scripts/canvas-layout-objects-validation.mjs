...
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${BUILD_ID}";`), "app build id should identify Iteration 54.3.7");
assert.ok(indexHtml.includes('const APP_ITERATION = "54.3.7";'), "visible iteration should be 54.3.7");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-3-7-device-editor-integration-hardening-modules";'), "module cache key should bust 54.3.7 modules");
assert.ok(bridgeSource.includes(`ENGINE_BRIDGE_VERSION = "${BUILD_ID}"`), "Engine bridge version should identify Iteration 54.3.7");
