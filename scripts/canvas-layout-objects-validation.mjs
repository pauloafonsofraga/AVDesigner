...
assert.ok(indexHtml.includes(`const APP_BUILD_ID = "${BUILD_ID}";`), "app build id should identify Iteration 54.9.1");
assert.ok(indexHtml.includes('const APP_ITERATION = "54.9.1";'), "visible iteration should be 54.9.1");
assert.ok(indexHtml.includes('const APP_MODULE_CACHE_ID = "iteration54-9-1-card-fan-out-edge-hardening-modules";'), "module cache key should bust 54.9.1 modules");
assert.ok(bridgeSource.includes('ENGINE_BRIDGE_VERSION = "iteration54-9-1-card-fan-out-edge-hardening"'), "Engine bridge version should identify Iteration 54.9.1");
